    ...
    void do_train_loop( vector<MCTNode*>& forward_result, VariableTensor &input_var, int NL )
    {
        ...
        ofstream outputfile("./bbb/bbb.out");
        ...
        for(int epoch=0;epoch<epoch_num;epoch++)
        {
            ...
            for(int j=0;j<n_batch;j++)
            {
                ...
                forward_result[376]->output = y_tmp;
                do_forward( forward_result, NL );
                
                auto o = forward_result[NL]->output;
                total_loss += o[0];
                
                int corrects = eval_labels( forward_result[351]->output, y_tmp );
            ...
            forward_result[376]->output = target_data;
            do_forward( forward_result, NL );
            
            auto o = forward_result[NL]->output;
            int corrects = eval_labels( forward_result[351]->output, target_data );
