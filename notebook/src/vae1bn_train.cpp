    ...
    void do_train_loop( vector<MCTNode*>& forward_result, VariableTensor &input_var, int NL )
    {
        ...
        fprec lr = 0.01;
      //int   NL =  88;  // loss value
        int   NZ =  39;  // latent value
        int   NS =  47;  // sigmoid
        int   NB =  69;  // binary cross entropy
        int   NK =  85;  // KL divergence
        ...
        cout<<"indata shape "<<indata_s[0]<<","<<indata_s[1]<<endl;
        ...
        for(int epoch=0;epoch<epoch_num;epoch++)
        {
            ...
            forward_result[68]->set_output1( (fprec)batch_size );
            forward_result[84]->set_output1( (fprec)batch_size );
            ...
            for(int jj=0;jj<n_batch;jj++)
            {
                ...
                //cout<<"epoch "<<epoch<<","<<jj<<" - loss "<<o<<" ( "<<o1<<" , "<<o2<<" ) "<<endl;
            ...
            forward_result[68]->set_output1( (fprec)indata_s[0] );  // div
            forward_result[84]->set_output1( (fprec)indata_s[0] );  // div
        ...
        {
            auto& rm = forward_result[7]->output;
            auto& rv = forward_result[8]->output;
            cout<<"run_mean"<<rm<<endl;
            cout<<"run_var "<<rv<<endl;
        }
